...
def create():
  ...
  def newline():
    ...
    def product():  
      ...
      txt = st.ScrolledText(Customerlabelframe, undo=True,width=62,height=4)
...
selectLabel = Button(midFrame,compound="top", text="Select",relief=RAISED, image=photo6,bg="#f8f8f2", fg="black", height=55, bd=1, width=55,command=select_record)
selectLabel.pack(side="left")

w = Canvas(midFrame, width=1, height=55, bg="#b3b3b3", bd=0)
w.pack(side="left", padx=5)

updateLabel = Button(midFrame,compound="top", text="Update",relief=RAISED, image=photo6,bg="#f8f8f2", fg="black", height=55, bd=1, width=55,command=update_record)
...
for record in fbcursor:
      
    invotree.insert(parent='',index='end',iid = count,text='',values=(record[0],record[1],record[3]))
       
    count += 1



invoiceid = Label(tab1,text="ID")
invoiceid.place(x=90,y=300)

invoicedate= Label(tab1,text="invoicedate")
invoicedate.place(x=160,y=300)

duedate = Label(tab1,text="duedate")
duedate.place(x=220,y=300)

id_entry = Entry(tab1)
id_entry.place(x=90,y=400)

date_entry = Entry(tab1)
date_entry.place(x=160,y=400)

due_entry = Entry(tab1)
due_entry.place(x=220,y=400)
...
#     set.insert(parent='',index='end',iid = count,text='',values=(id_entry.get(),date_entry.get(),due_entry.get()))
#     count += 1
...
tree = ttk.Treeview(tab1, columns = (1,2,3,4,5,6,7,8), show = "headings")
tree.pack(side = 'top')
tree.heading(1)
